  ...
  if [[ "${#visible_text_dirs[@]}" -gt 0 ]]; then
    while IFS= read -r -d '' file; do
      case "$file" in
        *.desktop)
          sed -i -E \
            -e '/^(Name|GenericName|Comment|Keywords)(\[[^]]+\])?=/ s/Linux Mint/Arcanus OS/g' \
            -e '/^(Name|GenericName|Comment|Keywords)(\[[^]]+\])?=/ s/linuxmint/Arcanus/g' \
            "$file"
          ;;
        *.ui|*.conf|*.xml|*.html)
          sed -i \
...
rebrand_iso_boot_files() {
  ...
  while IFS= read -r -d '' file; do
    # Check if file is actually text before attempting sed.
